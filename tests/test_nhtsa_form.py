...
def test_nhtsa_form_upload_and_metadata(client: TestClient, nhtsa_form_pdf_content: bytes):
    """Test uploading the NHTSA PDF form and verifying its metadata."""
    files = {"file": ("nhtsa_form.pdf", io.BytesIO(nhtsa_form_pdf_content), "application/pdf")}
...
def test_nhtsa_form_size_and_handling(client: TestClient, nhtsa_form_pdf_content: bytes):
    ...
    print(f"NHTSA form PDF size: {file_size:,} bytes ({file_size / (1024*1024):.2f} MB)")
...
def test_nhtsa_form_full_workflow(client: TestClient, nhtsa_form_pdf_content: bytes):
    ...
    if file_size >= max_size:
        pytest.skip(f"NHTSA form PDF ({file_size:,} bytes) exceeds 50MB limit - testing size rejection instead")
...
def test_nhtsa_form_structured_content_performance(client: TestClient, nhtsa_form_pdf_content: bytes):
...
def test_comprehensive_pdf_samples_all_types(
    ...
    assert len(set(fid for fid, _, _ in file_ids)) == len(file_ids)
    
    # Verify all can be retrieved and have valid metadata
    for file_id, pdf_type, file_size in file_ids:
        # Test retrieval
        pdf_response = client.get(f"/api/pdf/{file_id}")
        assert pdf_response.status_code == 200
...
def test_nhtsa_form_government_pdf_robustness(client: TestClient, nhtsa_form_pdf_content: bytes):
    ...
    for i in range(2):  # Upload, process, delete cycle 2 times
        files = {"file": (f"nhtsa_form_{i}.pdf", io.BytesIO(nhtsa_form_pdf_content), "application/pdf")}
