...
def test_anyline_sample_upload_and_metadata(client: TestClient, anyline_sample_pdf_content: bytes):
    """Test uploading the Anyline sample scan book PDF with complex images and barcodes."""
    files = {"file": ("anyline_sample.pdf", io.BytesIO(anyline_sample_pdf_content), "application/pdf")}
    ...
    assert metadata["page_count"] >= 1  # Should have multiple pages with scanned content
    assert not metadata["encrypted"]  # Should not be encrypted


def test_anyline_sample_complex_content_handling(client: TestClient, anyline_sample_pdf_content: bytes):
...
def test_anyline_sample_full_workflow(client: TestClient, anyline_sample_pdf_content: bytes):
    ...
    if file_size >= max_size:
        pytest.skip(f"Anyline PDF ({file_size:,} bytes) exceeds 50MB limit - testing size rejection instead")
...
def test_anyline_sample_image_rich_performance(client: TestClient, anyline_sample_pdf_content: bytes):
    ...
    assert upload_time < 60.0, f"Upload took {upload_time:.2f} seconds, which may be too slow for image-rich content"
...
def test_anyline_sample_barcode_content_robustness(client: TestClient, anyline_sample_pdf_content: bytes):
    ...
    for i in range(3):  # Upload, process, delete cycle 3 times
        files = {"file": (f"anyline_sample_{i}.pdf", io.BytesIO(anyline_sample_pdf_content), "application/pdf")}
